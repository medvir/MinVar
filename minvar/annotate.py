...
def find_frame(ref):
    ...
    if to_trim:
        return best_frame, refseq[best_frame - 1:-to_trim], aa_seq[best_frame].strip('*')
    else:
        return best_frame, refseq[best_frame - 1:], aa_seq[best_frame].strip('*')


def parse_cons_mutations(input_seq):
    '''Returns mutations of consensus w.r.t. consensus_B, 1-based coordinate on
    input sequence because we need to phase minority calls later
    '''

    Alignment.needle_align('asis:%s' % str(B_pol_nt.seq),
...
def parsevar(vcf_file, ref_file):
    ...
    for vc_line in vc_lines:
        ...
        if float(lsp[5]) < 100:
            warnings.warn('Quality filter on position %s: qual=%s' %
                (lsp[1], lsp[5]))
            continue
...
def merge_mutations(cons_muts, vcf_muts):
    '''Mutations found on sample consensus w.r.t. consensus B and minority ones
    found w.r.t. sample consensus must be merged to represent true state of
    variants w.r.t. consensus B. For all, position is on sample consensus

    e.g. let's consider
    positions    1 2 3 4 5 6 7
    consensus_B  A C T G A T T
                 | |   | | |
    sample cons  A C C G T T A
                 :   :   :
    vcf muts     C   G   A

    and let's assume that both vcf variants have 20% frequency.
...
def phase_mutations(muts, frame):
    ...
    for i in range(frame, 15000, 3):
        ...
        # if frameshift, nothing is saved
        if len(ins_muts) < 3:
            #warnings.warn('frameshift insertion detected at pos:%d-%d' % (i, i + 2))
            logging.info('frameshift insertion detected at pos:%d-%d' % (i, i + 2))
        # if in frame, save with mean frequency
        elif len(ins_muts) == 3:
...
def annotate_mutations(mutations, ref):
    '''Takes a DataFrame with nucleotide mutations and translates and annotates
    them according to coordinates in consensus B genes.
    Position on mutations is on ref (sample consensus), so we need to report
    these onto consensus_B.
    '''
    ...
    #assert isinstance(ref, SeqRecord)
    #frame, nt_framed, aa_framed = find_frame(ref)
    ref_nt = ref.seq
    B_nt = B_pol_nt.seq
    B_aa = B_pol_aa.seq
    ...
    for z in list(zip(ref, cur))[:end_pos]:
        ...
        cur_pos += z[1] != '-'
        if ref_pos != cur_pos:
            print(ref_pos, cur_pos, 'xyz', file=sys.stderr)
    ...
    for k, v in mutations.iterrows():
        ...
        if len(v.wt) == 1 and len(v.mut) == 1:  # v.wt can be XYZ after phasing
            assert B_nt[ref_pos - 1] == v.wt, '%s is not %s' % (B_nt[ref_pos - 1], v.wt)
        ...
        try:
            ...
            if '---' in mut_nt:
                mut_aa = ''.join([translation_table[str(mut_nt[i:i + 3])] for i in range(0, len(mut_nt), 3)])
            else:
                warnings.warn('CodonTable translation error: wt:%s pos:%d alt:%s' % (v.wt, v.pos, alt_nt))
                continue
        assert len(mut_aa) == len(B_aa), '%d is not %d' % (len(mut_aa), len(B_aa))
        ...
        if B_codon != mut_codon:
            ...
            if B_pos <= 56:
                gene_name = 'GagPolTF'
                gene_pos = B_pos
            elif 57 <= B_pos and B_pos <= 155:
                gene_name = 'protease'
                gene_pos = B_pos - 56
            elif 156 <= B_pos and B_pos <= 595:
                gene_name = 'RT'
                gene_pos = B_pos - 155
            elif 596 <= B_pos and B_pos <= 715:
                gene_name = 'RNase'
                gene_pos = B_pos - 595
            elif 716 <= B_pos and B_pos <= 1003:
                gene_name = 'integrase'
                gene_pos = B_pos - 715
...
def main(recalfile, ref_file='cns_final.fasta'):
    ...
    # consensus B pol gene
    ref_nt = list(SeqIO.parse(ref_file, 'fasta'))[0]
    frame, nt_framed, aa_framed = find_frame(ref_nt)
    logging.info('consensus reference file:%s\tframe:%d' % (ref_file, frame))
    logging.info('parsing mutations on consensus')
    cns_mutations = parse_cons_mutations(ref_nt)
    ...
    if vcf_mutations.shape[0]:
        ...
        vcf_mutations = vcf_mutations.groupby(['pos', 'wt', 'mut']).sum().reset_index()

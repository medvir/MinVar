...
def calc_APD(alignment):
    ...
    for index, row in APD_ambi_cons_freq_3rdcodon_df.iterrows():
        if row['cons_B'] != '-':
            break
        else:
            APD_ambi_cons_freq_3rdcodon_df.drop(index, inplace=True)    
    
    for index, row in APD_ambi_cons_freq_3rdcodon_df[::-1].iterrows():
        if row['cons_B'] != '-':
            break
        else:
            APD_ambi_cons_freq_3rdcodon_df.drop(index, inplace=True) 
    ...
    idx , ptr2 = 0, 0 
    amb_pos_count = 0
    while idx < APD_ambi_cons_freq_3rdcodon_df.shape[0]:
        #pointer2 = idx
        freq_ls = []
        while ptr2 < APD_ambi_cons_freq_3rdcodon_df.shape[0] and APD_ambi_cons_freq_3rdcodon_df.iloc[ptr2]['pos'] == APD_ambi_cons_freq_3rdcodon_df.iloc[idx]['pos']:
            freq_ls.append(APD_ambi_cons_freq_3rdcodon_df.iloc[ptr2]['freq'])
            ptr2 += 1
            
        freq_ls.append(1 - sum(freq_ls))
        freq_ls.sort(reverse = True)
        max_minority_freq = freq_ls[1]
        if max_minority_freq >= APD_THRESHOLD: # and APD_ambi_cons_freq_df.iloc[ptr2 - 1]['cons_B'] != '-': #and  snv_nogap_ref_df.iloc[pointer2 - 1]['amb'] != '-'
            #assert APD_ambi_cons_freq_3rdcodon_df.iloc[ptr2 - 1]['amb'] in wobbles, '%s in %d' % (APD_ambi_cons_freq_3rdcodon_df.iloc[ptr2 - 1]['amb'], ptr2 - 1)
